...
每个模块讲解完一个**知识点小节**后, **必须立刻出 3-5 个问题**检验用户是否掌握, 用户答完且**正确率达到 80% 以上**才能推进到下一个小节。

**问题结构**:
1. **是什么** (1 题, 必答): 检验概念理解
2. **为什么** (1-2 题, 必答): 检验设计决策理解
3. **怎么实现** (1-2 题, 必答): 检验代码细节理解
4. **对比/拓展** (选答): 关联已有知识

**评判标准**:
- 答对 ≥ 80% → 通过, 进入下一个知识点
- 答对 50-80% → 指出答错的部分, 重新讲解, 再问一轮
- 答对 < 50% → 重新讲整个知识点, 再问一轮

**禁止**:
- ❌ 用户说"懂了"就跳过检验
- ❌ 一次性问所有问题, 然后用户答完直接进下一个模块
- ❌ 用户没答对就放过

**示例**（模块 5 讲完 AutoGen SelectorGroupChat 后）:
```
好, AutoGen SelectorGroupChat 的基本原理讲完了。我出几道题检验一下你掌握没:

1. [是什么] SelectorGroupChat 里谁负责决定下一个 Agent 发言?
2. [为什么] 为什么不直接用 RoundRobinGroupChat, 让两个 Agent 轮流发言?
3. [怎么实现] 代码里用了哪个终止条件? 是 TextMentionTermination 还是 MaxMessageTermination? 为什么两个都用?
4. [对比] AutoGen SelectorGroupChat 跟 LangGraph 的 StateGraph 有什么区别?
